...
export default function Container(props: ContainerProps) {
  ...
  const [navbarVisible, setNavbarVisible] = useState(true); // Track navba
  const { children, ...customMeta } = props;
  const router = useRouter();
  const meta = {
    title: "DOTTEL",
    description: `From concept to launch, we deliver websites`,
    image: "/assets/ourlogo.webp",
    type: "website",
    ...customMeta,
  };
  ...
  useEffect(() => {
    const handleScroll = () => {
      setIsScrolled(window.scrollY > 0);if (window.scrollY > lastScrollY) {
        // Scrolling down
  ...
  return (
    <>
      <Head>
        ...
        <meta
          property="og:url"
          content={`https://www.dottel.in${router.asPath}`}
        />
        <link
          rel="canonical"
          href={`https://www.dottel.in${router.asPath}`}
        />
        <meta property="og:type" content={meta.type} />
        <meta property="og:site_name" content="Dottel" />
        <meta property="og:description" content={meta.description} />
        <meta property="og:title" content={meta.title} />
        <meta property="og:image" content="/assets/ourlogo.webp" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:site" content="Dotell" />
        <meta name="twitter:title" content={meta.title} />
        <meta name="twitter:description" content={meta.description} />
        <meta name="twitter:image" content="/assets/ourlogo.webp" />
        <link rel="manifest" href="/manifest.json" />
        <link rel="apple-touch-icon" href="/assets/ourlogo.webp" />
      </Head>
      <nav
  className={cn(
    styles.nav,
    isScrolled
      ? "bg-gradient-to-br from-background to-[rgba(0,0,0,0.8)] shadow-md backdrop-blur transition h-12"
      : "bg-[rgba(0,0,0,0.6)] h-12"
  )}
>
        ...
        <Link href="#home" className="flex items-center space-x-3 relative">
  <img
    className="h-10 w-8 rounded-full object-contain"
    src="/assets/Dottel.png"
    alt="Dottel Logo"

    style={{
      width: "100px", // Set the desired width of the logo
      height: "100px", // Set the desired height of the logo
    }}
              ...
              <div className="flex h-full flex-col items-start justify-between overflow-y-auto">
                ...
                <div className="flex min-h-fit w-full flex-col space-y-8 px-[22px] py-10">
                  <span className="text-sm text-muted-foreground">
                    © {new Date().getFullYear()} DOTTEL. All rights reserved.
                  </span>
                </div>
              </div>
            </motion.div>
